...
| DOC-04 | P1: Pipeline extrair→chunk→embed→indexar | Implemented | Implemented. ⚠️ **Defeito corrigido em 2026-09-06 (AD-057)**, e ele atingia esta feature também: `rag::pdfium::extract_text` rebindava a biblioteca a cada chamada, então o **segundo PDF importado em qualquer execução** falhava com `PdfiumLibraryBindingsAlreadyInitialized`. Achado pelo leitor, consertado na função compartilhada. **Ninguém reimportou dois PDFs pela tela de documentos para confirmar por este lado** — a tela não existe mais (AD-052); a prova é o teste `extracting_two_pdfs_in_the_same_process_reuses_the_bindings` |
