...
2. ⛔ **REVOGADO como estava escrito pela `book-reader` READ-32** (AD a registrar pela T12 da `book-reader`; implementado pela T17 em 2026-09-06). ~~WHEN um ou mais arquivos são escolhidos THEN o sistema SHALL copiar cada um para `<base_path>/library/`, preservando o nome original e acrescentando um sufixo numérico em caso de colisão~~
   **Passa a valer:** WHEN um ou mais arquivos são escolhidos THEN o sistema SHALL criar uma pasta por livro e SHALL copiar cada arquivo para `<base_path>/library/<pasta>/<arquivo>`, preservando o nome original **sempre**, e o sufixo numérico em caso de colisão SHALL recair sobre a **pasta**, não sobre o arquivo.
   **Por quê:** um diretório e um arquivo com o mesmo nome não coexistem no mesmo filesystem, então o livro só ganha pasta própria se o arquivo sair da raiz de `library/`. O critério original não foi apagado de propósito: o histórico do "por quê" tem valor (regra 4 de `.claude/rules/spec-driven-changes.md`).
...
⚠️ **A linha da lista deixou de ser só da `book-library` (2026-09-06).** As tasks T10 e T15 da `book-reader` acrescentaram ao `BookRow.tsx` o estado de processamento, os botões **Processar** / **Ler** / editar, a barra de progresso da tradução e o rótulo de formato não suportado — e a `LibraryPanel.tsx` passou a montar o `ProcessDialog` e o `BookEditPanel`. Os requisitos LIB-xx abaixo **continuam valendo como estão**; o que mudou é que os mesmos dois arquivos agora implementam READ-01 a READ-07 e READ-24 a READ-30 também, e os marcadores `SPEC:` deles listam as duas features. Nada de LIB-xx foi revogado por isso.
