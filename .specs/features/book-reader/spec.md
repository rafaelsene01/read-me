...
### P2: Ajustar o tamanho do texto (2026-09-12, READ-34, AD-076)

**User Story**: Como leitor, quero aumentar e diminuir a fonte dentro da leitura, para ler confortável.

**Acceptance Criteria**:

1. WHEN o usuário aciona "A" menor ou "A" maior no cabeçalho do leitor THEN o texto da página SHALL diminuir ou aumentar 10%, entre 70% e 200%, mostrando o percentual
2. The ajuste SHALL valer para página EPUB (inclusive livro com tamanhos em `px`) e para página de texto puro, e SHALL ser lembrado nesta janela
...
| READ-34 | P2: Aumentar/diminuir a fonte no leitor, 70%–200% (AD-076) | `ReaderPanel.tsx` — `zoom` no `body` do iframe (EPUB) e `fontSize` inline (texto puro); preferência em `localStorage` | implemented — **sem teste** (sem suíte de frontend); `npm run build` exit 0, i18n 237/237. **Não visto na tela:** o reflow com `zoom` num EPUB real e a leitura em voz alta marcando a frase certa com a página ampliada |
