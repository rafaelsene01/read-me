...
**Impact / o que NÃO foi verificado:** planejamento apenas — **nenhuma linha de código foi escrita ou alterada, nenhum gate rodou**. E há uma pré-condição que não é detalhe: nesta sessão, `src/`, `src-tauri/`, `scripts/` e `public/` estavam **apagados no working tree** (124 arquivos em `D`), presentes só no `HEAD`; todo o design foi lido via `git show HEAD:<arquivo>`. A T1 da `book-library` restaura a árvore e mede o baseline real. No mesmo levantamento apareceu uma divergência entre documentação e código: o `AGENTS.md` afirma que `src/types.ts` é gerado (`generated-types`) e que o frontend tem 63 testes (`frontend-testing`), mas **o `HEAD` não contém `src-tauri/src/types_export.rs` nem o script `test` no `package.json`**, e as duas pastas de spec estão untracked. As duas features estão documentadas como prontas e **não commitadas**.
...
**Impact / o que NÃO foi verificado:** a troca foi textual e no remote; **nada foi executado**. As árvores `src/`, `src-tauri/` e `scripts/` estão apagadas no working tree desta sessão, então não há `tauri.conf.json` para atualizar o `productName`, o `identifier` nem o `plugins.updater.endpoints` — quando esses arquivos voltarem, eles precisam ser conferidos um a um. Nenhum gate foi rodado (não há código para compilar). O repositório remoto no GitHub precisa existir com o nome novo; isso é do mantenedor.
