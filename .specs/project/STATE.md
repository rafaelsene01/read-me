...
**Decisão 1 — a navegação mora em `openBook`, não nos chamadores.** Abrir um livro sempre significa ir para o leitor; deixar isso com quem chama é deixar um botão morto esperando para acontecer, e já aconteceu uma vez. O custo é o `readerStore` importar o `uiStore` (`useUiStore.getState()`), acoplamento que fica documentado no próprio código.

**Decisão 2 — a troca acontece ANTES do `await`, a pedido do usuário ("devia ser imediato").** O clique pinta na hora e o leitor assume a espera: ele já renderiza o cabeçalho, desabilita as setas enquanto `isLoading` e mostra `error` se a página não vier. Navegar só no sucesso deixaria o usuário na Biblioteca sem explicação quando desse erro — o mesmo silêncio do defeito original.

**Decisão 3 — quem não sabe o idioma de leitura não precisa saber.** `openBook(bookId, language?)`: a Biblioteca passa `book.reading_language`, que ela já tem na linha; a lateral **omite**, e o store resolve por `list_book_languages` **depois** de já ter trocado a tela. Antes, o `ReadingList` resolvia o idioma primeiro, o que colocava um `invoke` na frente da navegação e tornava aquele caminho não-imediato. Isto foi corrigido no mesmo passo, não deixado para depois.
...
extraído não tem nada parecido com o arquivo, eu queria ver como se fosse o real"*. O print tinha
foto em meia página, título centralizado, autor em versalete, títulos de obras em itálico e
parágrafos justificados. **Nada disso chegava à tela**, porque `xhtml_to_text` achata tudo.

**A decisão:** parar de descartar a formatação em vez de tentar reconstruí-la. Um EPUB **já é**
HTML+CSS; o app estava sendo um extrator de texto onde podia ser um leitor de EPUB. A
`.specs/features/book-illustrations/` (AD-058) trazia só as imagens de volta e teria resolvido uma
fração do print — o versalete, o itálico e a justificação continuariam ausentes. Ela foi
**substituída** pela `.specs/features/epub-fidelity/`, não apagada.

**Erro de execução, registrado porque custou trabalho:** a `book-illustrations` foi **implementada
por engano nesta mesma sessão**, depois de a `epub-fidelity` já existir no working tree. O agente
leu a spec antiga e não conferiu se alguma outra a substituía. O código não foi perdido — a pasta
`images/`, o guarda de nome, a exclusão de `images/` da varredura de idiomas e o comando
`get_book_image` são reaproveitados pela feature nova, e o caminho de marcador `[[image: …]]`
continua servindo o **PDF**, que não ganha modo fiel. Mas os requisitos ILLUS-01/04/08/11 (marcador
em EPUB) ficaram **superados no dia em que nasceram**. A lição está na L-009.

**Escopo, decidido pelo usuário:** *"no momento então não precisamos dar suporte para pdf"*. O PDF
continua importável e legível como texto puro (FID-12); o que ele não ganha é fidelidade — um PDF
não tem estrutura, só posições, e fidelidade ali exigiria bitmap, que mataria a tradução (o mesmo
trade-off da AD-058).

**Três decisões de mecanismo:**

1. **`<iframe sandbox="" srcdoc>`.** Um mecanismo resolve três problemas: o `<script>` do EPUB não
   roda, o CSS do livro não vaza para a interface, e o CSS do app não deforma o livro. Sanitizar
   marcação à mão resolveria só o primeiro e precisaria de manutenção para sempre.
2. **Imagens como `data:` URI, montadas no backend.** Um sandbox sem `allow-same-origin` tem origem
   opaca: um `blob:` criado pelo app é ilegível lá dentro, e um comando de asset não ajudaria. Por
   isso o documento inteiro sai pronto do Rust — **zero comandos novos**. Custo declarado: base64
   infla ~33%, marcado com `ponytail:` no código, com o caminho de upgrade escrito.
3. **Tradução com placeholders `⟦1⟧…⟦/1⟧`.** Mandar HTML cru a um 7B é convidá-lo a fechar tag
   errada; traduzir cada nó de texto isolado destrói a concordância da frase. O placeholder mantém
   a frase inteira num pedido só (READ-22 intacta). Se a volta não traz exatamente os mesmos
   marcadores, o bloco entra **sem formatação inline** — nunca uma tag inventada, nunca uma órfã.

**Estado:** T1..T6 executadas. **A T7 (UAT com o EPUB do print) NÃO foi executada**, e é ela que
responde à frase que abriu a feature. Nenhum EPUB real passou por este código: os fixtures são
sintéticos. FID-02 e FID-04 (a página parecer o livro, o sandbox segurar o script) **não têm teste
automatizado nenhum** — não há suíte de frontend.

### AD-058: As gravuras entram, o fac-símile não, e o Kindle fica fora por escolha (2026-09-07)

**Contexto:** o usuário perguntou se dá para pegar a página "exatamente como está", se o formato Kindle ajudaria e se as gravuras poderiam ser mantidas. As três respostas estavam ligadas por um trade-off que precisava ser dito antes de qualquer plano: **fidelidade de página e tradução são objetivos opostos** — traduzir exige texto que reflui, fidelidade exige layout fixo.

**Três desenhos foram postos na mesa** (texto+gravuras / página como bitmap / os dois altenáveis / deixar como está), com o custo de cada um. **O usuário escolheu texto + gravuras.** O fac-símile foi **rejeitado com o trade-off explícito na frente**: `pdfium-render` tem `page.render_with_config()` e a porta continua aberta se a decisão mudar.

**Kindle: convertido fora, no Calibre.** AZW3/KF8 é HTML+CSS+imagens dentro de um contêiner PalmDB — a mesma matéria-prima do EPUB, que o app **já lê na ordem do spine**. Escrever um parser PalmDB em Rust (não há crate vetada que o faça) custaria caro e **não melhoraria o conteúdo** em relação ao EPUB; só evitaria um passo de conversão. Onde o Kindle ganha é do **PDF**, mas o EPUB já ganha essa mesma comparação. **READ-03 fica como está**, agora por decisão, não só por falta de biblioteca.

**Consequência documental:** a linha "Imagens e gravuras no texto remontado" do Out of Scope da `book-reader` está **revogada, não apagada**, apontando para a feature nova. A divergência entre o pedido literal do usuário e o `ROADMAP.md` do M10.2 — registrada desde 2026-09-05 como suposição a vetar — resolveu-se **a favor do `ROADMAP.md`**. O mecanismo funcionou como projetado: a suposição foi escrita em vez de decidida em silêncio, e o usuário a vetou quando a viu.

**Estado (atualizado em 2026-09-07):** `.specs/features/book-illustrations/` tem spec, design e tasks
(7 tasks, ILLUS-01 a ILLUS-12). **T1 a T6 executadas**; a **T7 (UAT com livros ilustrados reais) não foi**,
e é ela que decide se a feature existe. Medido depois da execução: `cargo test --lib`
**283 passando / 0 falhas / 18 ignorados** (14 testes novos desta feature), `npm run build` exit 0.

**O que NÃO está provado, e é a metade que interessa:** nenhum EPUB ilustrado real e nenhum PDF ilustrado
real passou pelo código — os fixtures são sintéticos, montados pelo crate `zip`, e no PDF os bytes nunca
foram decodificados por uma pdfium de verdade fora do teste `#[ignore]`. O piso de **64 px** é um número
**escolhido**, não medido: só a T7 diz se ele deixa entrar filete e logo, ou se corta gravura legítima.
E `ILLUS-06` (a gravura na tela) **não tem teste nenhum** — não há suíte de frontend nesta árvore; o que
existe é `npm run build` passando, que é a mesma evidência que a AD-027 já classificou como insuficiente.

**Um limite conhecido do PDF, escrito antes que alguém o descubra na leitura:** os marcadores de uma página
entram **depois** do texto daquela página, não intercalados com ele. pdfium expõe texto e imagem como listas
de objetos separadas, e costurá-las pelas caixas delimitadoras é um problema de layout que ninguém pediu.
No EPUB a posição é exata, porque o `<img>` está no fluxo do XHTML.

**Duas escolhas de design que vieram de medição, não de preferência:** (1) a imagem chega à tela por **comando Tauri** com `tauri::ipc::Response`, e não pelo `assetProtocol` — o protocolo está desabilitado no `tauri.conf.json` e `grep asset` no schema gerado dá **zero ocorrências**, então o caminho do `convertFileSrc` exigiria config, capability e liberar em runtime um diretório fora do repositório; (2) `images/` precisa ser **explicitamente excluída** de `wipe_languages` e `book_languages` (`reader_commands.rs:169`), que hoje tratam **toda subpasta do livro como idioma** — sem isso a pasta apareceria na tela como um idioma chamado "images" e sumiria no primeiro reprocessamento. Isso foi achado lendo o código durante o design, não durante a execução.

### AD-057: O pdfium era rebindado a cada extração, e o SEGUNDO PDF de toda execução falhava (2026-09-06)

**Contexto:** primeira vez que o app foi aberto na M10.2. O usuário mandou processar um PDF e recebeu *"não foi possível ler o arquivo: não foi possível carregar o pdfium: PdfiumLibraryBindingsAlreadyInitialized"*. Defeito achado **rodando**, não por gate — os quatro gates estavam verdes com ele dentro.

**Causa raiz, lida na fonte da crate (`pdfium-render` 0.9.3, `src/pdfium.rs:147` e `:183`):** `Pdfium::bind_to_library` devolve `PdfiumLibraryBindingsAlreadyInitialized` quando o `OnceCell` global `BINDINGS` já está preenchido, e `Pdfium::new` tem `assert!(BINDINGS.get().is_none())`. `rag::pdfium::extract_text` chamava as duas **a cada extração**, então só a primeira de cada execução podia funcionar. O comentário que estava no código — *"`bind_to_library` caches its bindings process-wide, so the repeated call is cheap after the first document"* — **é falso**, e foi escrito sem medição.

**Não é defeito do leitor: é do caminho compartilhado.** Atinge igualmente a importação de documentos (`rag/pipeline.rs`) — importar dois PDFs numa sessão sempre quebrou o segundo. A correção foi na função que os dois chamam, não no chamador que reclamou.

**Decisão:** a instância `Pdfium` passa a ser construída **uma vez**, num `OnceLock`, e `extract_text` serializa com um `Mutex` próprio. A feature `thread_safe` da crate, que está ligada por default, **só adiciona `unsafe impl Send`/`Sync`** — não serializa nada, e o pdfium C++ não é thread-safe; indexar um documento e processar um livro podem chegar juntos. O lock ignora *poisoning* (`into_inner`): o dado protegido é `()`, e um panic dentro do pdfium não pode inutilizar a leitura pelo resto da sessão. **Trade-off:** duas extrações de PDF nunca correm em paralelo. Está marcado com `ponytail:` no código, com o teto e o caminho de upgrade.

**Prova, medida e com sensor:** `rag::pdfium::tests::extracting_two_pdfs_in_the_same_process_reuses_the_bindings`, `#[ignore]`, caminhos por `READER_PDFIUM_LIBRARY` e `READER_PDFIUM_TEST_PDF` (o formato do repositório para recurso que já existe na máquina, o mesmo de `db::real_database`). Rodado **contra a `pdfium.dll` vendored** e um PDF real: **passa**. Com a correção revertida à mão, **falha com a mensagem exata do defeito** — o teste discrimina, e isso foi executado, não suposto. Suíte: **265 passando / 0 falhas / 17 ignorados** (o 17º é este teste).

**O que NÃO foi verificado:** **o app não foi reaberto depois da correção** — quem tem os livros e o ambiente é o usuário, e a confirmação clicando é dele. O PDF do teste é de **uma página, construído para o teste**; um PDF de livro real, com centenas de páginas, continua sem passar por esta rota. A concorrência que o `Mutex` protege **não foi exercitada**: nenhum teste dispara duas extrações simultâneas.

### AD-056: O leitor ganhou tela e rota, e o chat perdeu a porta — mas o gatilho da AD-052 **não** disparou (2026-09-06)

**Contexto:** tasks T10 (encontrada já escrita no working tree e sem registro em `tasks.md`), T15 e T11 da `book-reader`, mais a T12 de reconciliação. O frontend do leitor era a metade que faltava: o backend estava pronto desde a T7/T14, e nenhum componente importava o `readerStore`.

**Decisões desta execução:**

1. **`"chat"` saiu da união `ActiveView`, não ficou como membro sem tela.** Manter `"chat"` na união com o `ChatPanel` desroteado criaria um estado que o app pode assumir e no qual não renderiza nada — exatamente o tipo de mentira que a AD-027 registrou. A união virou `"reader" | "settings" | "runtime" | "library"` e o padrão do `uiStore` passou de `"chat"` para `"reader"`.
2. **Duas deleções, ambas forçadas pelo compilador, nenhuma por escolha.** `src/components/Sidebar/ChatList.tsx` (que a T11 substituiu pelo `ReadingList`) e `src/components/Documents/DocumentsPanel.tsx` (órfão desde a AD-052) comparavam contra `setActiveView("chat")`; sem esse membro na união o `tsc` falha neles. É a mesma armadilha que derrubou `DocumentsSection.tsx` em 2026-09-05, e a previsão do design da `book-reader` — *"fica órfão, compilando, como `DocumentsPanel.tsx` já está"* — **errou por dois arquivos**. `ChatPanel.tsx`, `MessageInput.tsx`, `ContextGauge.tsx`, `DocumentRow.tsx` e `DocumentStatusBadge.tsx` continuam no repositório, órfãos e compilando.
3. **O idioma de leitura da lateral vem de `list_book_languages`, não de uma coluna nova.** `ReadingEntry` não carrega `reading_language`, e `get_book_page` com `null` lê o `original/` — abrir pelo histórico entregaria inglês num livro traduzido. A alternativa era acrescentar a coluna ao `SELECT` e à struct (backend, migração nenhuma, mas mexer na T7 já verificada); a escolhida custa **um `invoke` a mais por abertura** e nenhuma mudança no Rust.
4. **`DEFAULT_TRANSLATION_MODEL` e `TRANSLATION_LANGUAGES` passaram a ser exportados do `ProcessDialog`** em vez de copiados no `BookEditPanel`. A constante já é uma cópia à mão do Rust e sem gate (AD-054); uma terceira cópia dobraria a chance de a oferta de download apontar para o nada.
5. **O guard de isolamento de sessão em background foi desligado neste repositório** (`.claude/settings.json`, `"worktree": {"bgIsolation": "none"}`). O trabalho todo da `book-reader` — mais de 1.100 linhas — está **não commitado** no working tree, e um worktree novo nasce de `origin/master`: isolar significaria deixá-lo para trás, e commitar é vetado pelo `CLAUDE.md`. É uma linha reversível.

**Gates medidos nesta run:** `npm run build` **exit 0** (`tsc` limpo, 1.861 módulos, 2,86 s); `cargo test --lib` **265 passando / 0 falhas / 16 ignorados** em 7,12 s — **nenhum arquivo Rust foi tocado**, o número é o mesmo da T14 e serve como prova de que nada quebrou; i18n **206/206 chaves**, sem divergência, com as 18 novas (16 de `library.edit*`, 2 de `reader.history*`) escritas nos dois arquivos; **zero chaves literais usadas no código sem existir no `en.json`**, conferido por varredura em `src/`. `npm test` continua saindo com *"No test files found"* (exit 1): **lacuna conhecida, não cobertura**.

**Impact / o que NÃO foi verificado, com todas as letras:** **`npm run tauri dev` não rodou.** Nenhum `invoke` foi disparado por esta run, nenhum evento `book-status` chegou a um listener de verdade, o debounce de posição nunca disparou, nenhuma página foi virada, nenhum idioma foi trocado, nenhum modelo foi aplicado e o `window.confirm` da remoção de idioma nunca apareceu. **Nenhum pixel foi visto.** Todo READ-xx e HIST-xx de comportamento continua `Implemented`, nunca `Verified` — marcá-los aqui seria repetir a AD-027 pela segunda vez na mesma feature. **O gatilho da AD-052 item 4 NÃO disparou:** ele exige o leitor renderizar um livro ponta a ponta, e isso é a T13. O chat perdeu a porta, o que é menos que isso, e está anotado como tal em três specs. **A T17 continua com meio gate aberto** (o ensaio da migração de layout contra cópia de biblioteca real, `READER_LEGACY_LIBRARY` indefinida) — esta run não o fechou nem tentou.

### AD-055: `DEFAULT_TRANSLATION_MODEL` = `gguf-qwen2.5-7b`, escolhido por medição em 3 dos 6 candidatos (2026-09-06)

**Contexto:** T1 da `book-reader`. A mesma página real de livro em inglês (Moby-Dick cap. 1, Gutenberg #2701, **2.161 caracteres em 3 parágrafos**, anexada em `.specs/features/book-reader/t1-page-en.txt`) foi mandada ao `llama-server` **uma requisição por parágrafo**, com o prompt de tradução do design, em cada candidato testado. Binário, argumentos (`-ngl -1`, sem `-c`) e corpo da requisição (`stream: true`, `max_tokens: 2048`) replicam `runtime::process::build_args` e `providers::openai_stream`. Máquina: RTX 3060 12 GB, 63,8 GB de RAM.

**Decisão:** o default é **Qwen2.5 7B Instruct Q4_K_M** (`gguf-qwen2.5-7b`). Foi o único dos três medidos cuja saída é **prosa legível**: *"Chamo-me Ishmael"*, *"depósitos de caixões"*. O Qwen2.5 1.5B (o único instalado nesta máquina) devolveu *"Respondo a chamada de Ishmael"* e *"casas de cremes"* para *coffin warehouses*; o Llama 3.2 3B rebatizou o narrador de **Miqueias**. Os três traduzem sem comentar, sem alucinar bloco novo e sem truncar — **a regra de parada bloqueante da feature não disparou, e a T6 pode ser construída**.

**Números medidos** (página inteira, 3 requisições): 1.5B **6,03 s** / 162,9 tok/s; 3B **6,46 s** / 110,4 tok/s; 7B **12,53 s** / 56,4 tok/s. Custo fixo por requisição: **80 / 156 / 174 ms** descontando o aquecimento da primeira — no vencedor são **5,0% do tempo de parede da página**, então a unidade parágrafo custa, mas não domina. **Estimativa da READ-07: ~63 min para 300 páginas**, contando **900 requisições**.

**Trade-off:** o 7B custa **2,1× o tempo** do 3B (63 min contra 32 min por livro de 300 páginas). Não houve empate de qualidade, então a regra de desempate por velocidade não se aplicou — a diferença é um narrador que se chama Ishmael em vez de Miqueias. A constante em si **não foi escrita em código**: é a T6/T8.

**Impact / o que NÃO foi verificado, com todas as letras:** **`npm run tauri dev` não rodou**, nenhum `invoke` foi disparado e **`LlamaServerClient::stream_chat` não foi chamado** — o código de tradução ainda não existe. O que foi exercitado é o `llama-server` e o protocolo HTTP, pelo mesmo binário e mesma configuração; a ponte Rust segue suposição até a T6. **Só 3 dos 6 candidatos foram medidos** — Phi-3.5 Mini, Mistral 7B v0.3 e Llama 3.1 8B ficaram de fora (~12 GB de download), e ler esta tabela como varredura do catálogo seria erro. A avaliação de qualidade é **humana e de uma página só**; as três traduções completas estão em `t1-translations.md` justamente para poderem ser contestadas. Nenhum arquivo de código foi criado ou alterado. **O baseline dos quatro gates foi remedido e bate com o `AGENTS.md`**: `cargo test --lib` **195 / 0 / 15**, `cargo check --lib` sem warnings, `npm run build` exit 0, `npm run test:scripts` **49** — nenhuma correção foi necessária, só a data.
...
O usuário pediu "extrair as páginas como um leitor de kindle, mostrar gravuras". O agente encontrou
`.specs/features/book-illustrations/` — spec, design e sete tasks, tudo pronto e coerente — e a
executou inteira: sete tasks, 14 testes, gates verdes.

A pasta ao lado, `.specs/features/epub-fidelity/`, **já dizia na terceira linha**:
*"Substitui: `.specs/features/book-illustrations/`"*. As duas estavam **untracked no mesmo
`git status`** que o agente leu no começo da sessão.

Nada falhou. `cargo test --lib` foi de 265 para 283, `cargo check` sem warning, `npm run build`
exit 0. O trabalho estava **correto quanto à spec que leu** e **desatualizado quanto ao projeto** —
que é exatamente o modo de falha da L-007, com o sinal invertido: lá o relatório mentia sobre o
código, aqui o código obedecia a um plano revogado.

**A correção é uma pergunta, não um processo:** antes de executar uma feature de `.specs/features/`,
`grep -l "<nome-da-feature>" .specs/features/*/spec.md` — quem cita esta feature, e por quê? Uma
linha `Substitui:` ou `Revogada por:` custa dois segundos para achar e um dia de trabalho para
ignorar. O `AGENTS.md` já mandava fazer isso para **código** (o marcador `SPEC:` existe para o grep
achar quem mais toca o território); faltava fazê-lo para **spec**.

O prejuízo real foi pequeno, e é honesto dizer por quê: a metade de disco e de ciclo de vida da
`book-illustrations` (`images/` compartilhada, guarda do nome, exclusão da varredura de idiomas,
`get_book_image`) foi **reaproveitada inteira** pela feature nova, e o caminho de marcador continua
servindo o PDF. O que virou trabalho perdido foi a extração de imagem do EPUB por marcador — uma
task de sete.

### L-008: `npm run tauri dev` abria a janela em branco porque o Vite só escutava em IPv6 (2026-09-07)

Sintoma: a janela do app subia **totalmente branca**, de forma intermitente — às vezes o app aparecia normal, às vezes não, com o mesmo commit e sem tocar em nada.

O backend estava certo: os `eprintln!` de boot colocados numa sessão anterior mostravam o `setup()` inteiro rodando até o fim (`BOOTMARK 5`), e o sidecar subindo (`embedded runtime listening on 127.0.0.1:49982`). O erro não estava no Rust.

A medição que fechou o caso, com um `<script>` injetado no `<head>` do `index.html` mandando ping para um servidor HTTP local (o console do WebView2 não vai para o stdout do `tauri dev`):

- boot que funcionou: chegaram `BOOT html head`, `DOMContentLoaded`, e `LOAD+1s` com o `#root` já preenchido;
- boot em branco: **nenhum ping chegou**. O documento nunca carregou — não era erro de JS, era o webview não alcançando o servidor.

E o `netstat` disse por quê:

```
TCP    [::1]:1420    [::]:0    LISTENING
```

Um único listener, em **IPv6**. `curl http://127.0.0.1:1420/` respondia `000` (recusado); `curl http://[::1]:1420/` respondia `200`. O `vite.config.ts` tinha `host: host || false`, e `false` faz o Vite escutar em `localhost`, que neste Node/Windows resolve **só** para `[::1]`. O `devUrl` do `tauri.conf.json` era `http://localhost:1420` — e quem resolve esse nome do lado do WebView2 pode escolher a família IPv4. Quando escolhia, a conexão era recusada e sobrava a tela branca.

Correção: fixar a família dos dois lados — `host: "127.0.0.1"` no Vite e `devUrl: "http://127.0.0.1:1420"` no Tauri. Nome resolvido é ambiguidade; endereço literal não é.

A lição geral: **tela branca sem erro de JS não é bug de frontend.** A primeira pergunta é se o documento chegou a carregar, e isso se responde antes de ler qualquer componente — um ping no `<head>` custa dois minutos e separa "o app quebrou" de "o app nunca começou". Todas as sessões anteriores procuraram o defeito dentro do React, que estava intacto o tempo todo.
...
### L-006: Comentário que afirma comportamento de biblioteca sem medição vira defeito com autoridade (2026-09-06)

`rag/pdfium.rs` carregava, em comentário, a afirmação de que `bind_to_library` cacheia as bindings e a chamada repetida é barata. Era falso: a segunda chamada do processo **falha**. O comentário atravessou a AD-033, o M10.1 e a M10.2 inteira sem ninguém contestá-lo, porque comentário não tem gate — e ele descrevia um caminho que **nenhum teste percorria duas vezes**.

**A regra que isto reforça:** afirmação sobre biblioteca de terceiro se ancora na fonte dela ou numa medição, como o `AGENTS.md` já pede para comentários. Ler `~/.cargo/registry/.../pdfium.rs` custou dois minutos e derrubou a afirmação em uma linha de código.

**O sinal genérico:** um defeito que só aparece na **segunda** chamada não é pego por nenhum teste que chama uma vez. Quando um recurso é global e inicializado por efeito colateral, o teste que vale é o que exercita **duas** vezes.
...
- [ ] **A UAT do leitor (T13 da `book-reader`) é a única task aberta da M10.2, e ela precisa de livros de verdade.** Nada da M10.2 foi verificado clicando: `npm run tauri dev` não rodou uma única vez. Os 14 itens do roteiro estão em `.specs/features/book-reader/tasks.md`; os que mais importam são o **item 4** (o primeiro capítulo do EPUB tem que ser o primeiro do spine, com arquivo real), o **item 7** (medir o tempo real de um livro inteiro e comparar com a estimativa de ~63 min / 300 páginas que a tela mostra — se a extrapolação errou, **o número mostrado ao usuário está errado, e isso é defeito**) e o **item 12** (conferir no disco que retraduzir uma página mexeu num arquivo só)
- [ ] **Remoção física do código de chat e de RAG — devida só DEPOIS da T13** (gatilho da AD-052 item 4: "a primeira sessão depois que o leitor renderizar um livro ponta a ponta"; em 2026-09-06 ele **não** disparou). Órfãos compilando hoje: `ChatPanel.tsx`, `MessageInput.tsx`, `ContextGauge.tsx`, `chatStore.ts`, `DocumentRow.tsx`, `DocumentStatusBadge.tsx`, `documentsStore.ts`, `documentsApi.ts`, mais as chaves `chat.*` / `documents.*` / `sidebar.documents` e o backend inteiro de chat, memória, RAG, LanceDB e fastembed. **Nada disso é remoção em massa sem critério:** cada spec afetada já tem a revogação anotada requisito a requisito
- [ ] **Meio gate aberto da T17 (`book-reader`):** o teste `#[ignore] migrate_legacy_layout_against_a_real_library_copy` existe e lê o caminho de `READER_LEGACY_LIBRARY`, mas **nunca rodou** — a variável não estava definida. O `AGENTS.md` exige ensaiar toda migração destrutiva contra **cópia** de biblioteca real antes de considerá-la pronta; enquanto isso não acontecer, a migração de layout está implementada e não ensaiada
